...
class UserController extends AdminController {
    ...
    */
    ...
    { 
        $users = User::select(array('users.id','users.name','users.username','users.email','users.admin','users.confirmed', 'users.created_at'));

        return Datatables::of($users)
            ->edit_column('admin', '@if ($admin=="1") {{trans("admin/users.admin_1")}} @elseif ($admin=="2") {{trans("admin/users.admin_2")}} @elseif ($admin=="3") {{trans("admin/users.admin_3")}} @endif')
